...
def simulate_quadrotor(x0, tf, quadrotor,N=10,use_mpc=True, use_mpc_with_clf=False, use_clf_qp=False):
  ...
  while np.linalg.norm(np.array(x[-1][0:2])) > 1e-3 and t[-1] < tf:
    ...
    if use_mpc:
      current_u_command = quadrotor.compute_mpc_feedback(current_x,current_time,N,use_mpc_with_clf)
